...
func (t *UploadTask) handleSegmentProcess() error {
	...
	for _, segment := range segments {
		...
		// 强制触发发送最近的节点
		if err := t.ForceSendClosest(segment.SegmentId); err != nil {
			logger.Errorf("发送最近的节点失败: segmentID=%s, err=%v", segment.SegmentId, err)
			continue
		}
	}
...
// 返回值:
//   - error: 如果处理过程中发生错误，返回相应的错误信息
func (t *UploadTask) handleSendClosest(segmentID string) error {
	logger.Infof("发送最近的节点请求: segmentID=%s", segmentID)
	...
	if !exists {
		logger.Warnf("片段记录不存在: segmentID=%s", segmentID)
		return fmt.Errorf("片段记录不存在: segmentID=%s", segmentID)
	}
	...
	if len(segment.SegmentContent) == 0 {
		logger.Errorf("分片内容为空: segmentID=%s", segment.SegmentId)
		return fmt.Errorf("分片内容为空: segmentID=%s", segment.SegmentId)
	}
	...
	if err != nil {
		...
		logger.Errorf("发送最近的节点请求失败: segmentID=%s, err=%v", segmentID, err)
		return err
	}
	...
	logger.Infof("成功发送片段到节点: peerID=%s, segmentID=%s", sendResult.Target, segmentID)
	...
	t.NotifySegmentStatus(&pb.UploadChan{
		...
		SegmentSize:    segment.Size_,                     // 设置分片大小
		IsRsCodes:      segment.IsRsCodes,                 // 设置是否使用纠删码
		NodeId:         sendResult.Target.String(),        // 设置存储节点ID
		UploadTime:     time.Now().Unix(),                 // 设置当前时间戳
	})
