...
type UploadTask struct {
	...
	// 任务控制通道
	chPause  chan struct{} // 暂停：暂停当前上传任务
	chCancel chan struct{} // 取消：取消当前上传任务
	chDelete chan struct{} // 删除：删除当前上传任务及相关资源
}
...
func NewUploadTask(ctx context.Context, opt *fscfg.Options, db *database.DB, fs afero.Afero,
	...
	return &UploadTask{
		...
		// 任务控制通道
		chPause:  make(chan struct{}, 1), // 暂停：暂停当前上传任务
		chCancel: make(chan struct{}, 1), // 取消：取消当前上传任务
		chDelete: make(chan struct{}, 1), // 删除：删除当前上传任务及相关资源
	}
}
