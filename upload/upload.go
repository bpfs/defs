...
func Upload(
	...
	fileInfo, fileHash, err := readFile(opt, file, pubKeyHash)
	if err != nil {
		return nil, err
	}
	...
	if err := readSplit(file, fileInfo, fileHash, fileInfo.GetSize()+opt.GetDefaultBufSize(), dataShards, parityShards); err != nil {
		return nil, err
	}

	go func() {
		// 创建文件处理函数
		if err := createFileWith(pool, registry, fileInfo, dataShards, parityShards); err != nil {
...
// 创建文件处理函数
func createFileWith(pool *pool.MemoryPool, registry *eventbus.EventRegistry, fileInfo *core.FileInfo, dataShards, parityShards int64) error {
	// 添加一个新的上传任务到内存池
	...
	// 本地文件片段
	if err := localFileFragments(pool, registry, fileInfo, fpath); err != nil {
		return err
	}
...
func localFileFragments(pool *pool.MemoryPool, registry *eventbus.EventRegistry, fileInfo *core.FileInfo, fpath string) error {
	// 获取文件片段上传事件总线
	...
	for k, slice := range fileInfo.GetSliceList() {
		...
		// 文件片段存储为本地文件
		if err := sliceLocalFileHandle(fileInfo, slice, fpath); err != nil {
			return err
		}
...
func sliceLocalFileHandle(fileInfo *core.FileInfo, s core.SliceInfo, fpath string) error {
	// 将 size 转换为 []byte
