...
// readSplit 读取分割
func readSplit(r io.Reader, fileInfo *core.FileInfo, fileHash []byte, capacity, dataShards, parityShards int64) error {
	// 使用子函数读取数据到 buffer。
	...
	for index, sliceHash := range sliceHashes {
		...
		// 根据给定的私钥和数据生成签名
		signature, err := generateSignature(privateKey, fileInfo.GetFileID(), fileInfo.GetSliceTable(), index, sliceHash, rc)
		if err != nil {
			return err
		}
		// 更新 FileInfo 的片段列表
...
func generateSignature(privateKey *rsa.PrivateKey, fileID string, sliceTable map[int]core.HashTable, index int, sliceHash string, rc bool) ([]byte, error) {
