...
func main() {
	...
	//time.Sleep(60 * time.Second) // 延时30秒

	//address := "12gpXQVcCL2qhTNQgyLVdCFG2Qs2px98nV"
	path := "BPFS 白皮书.pdf"
	// file, err := upload.Upload(fs.Ctx(), fs.Opt(), fs.P2P(), fs.Pubsub(), fs.DB(), fs.UploadChan(), fs.DownloadChan(), fs.Registry(), fs.Cache(), fs.Pool(), path)
	file, err := upload.Upload(fs.Ctx(), fs.Opt(), fs.DB(), fs.Registry(), fs.Cache(), fs.Pool(), path, privateKey)
	if err != nil {
	...
	logrus.Printf("FileKey:\t%v\n", file.FileKey)
	logrus.Printf("Name:\t%v\n", file.Name)
	logrus.Printf("Size:\t%v\n", file.Size)
	logrus.Printf("ModTime:\t%v\n", file.ModTime)
	logrus.Printf("UploadTime:\t%v\n", file.UploadTime)
	logrus.Printf("FileType:\t%v\n\n", file.FileType)

	logrus.Printf("开始下载任务\t%s\n\n", file.FileID)
	pubKeyHash, err := wallet.GetPubKeyHashFromPrivKey(privateKey) // 从ECDSA私钥中提取公钥哈希
	if err != nil {
		return
	}

	if err := download.Download(fs.DB(), fs.Registry(), fs.Pool(), file.FileID, file.FileKey, pubKeyHash); err != nil {
	...
	// if err := edit.EditName(fs.P2P(), fs.Pubsub(), fs.DB(), file.FileID, privateKey, newName); err != nil {
	// 	logrus.Errorf("Download 报错:\t%v", err)
	// 	return
	// }
	...
	// if err := edit.EditShared(fs.P2P(), fs.Pubsub(), fs.DB(), file.FileID, file.FileKey, true, privateKey); err != nil {
	// 	logrus.Errorf("Download 报错:\t%v", err)
	// 	return
	// }
	// logrus.Printf("开始新增分享任务\t%s\n\n", file.FileID)
	// userPubHash, err := script.GetPubKeyHash(address)
	// if err != nil {
	// 	return
	// }
	...
	// if err := edit.AddShared(fs.P2P(), fs.Pubsub(), fs.DB(), file.FileID, file.FileKey, userPubHash, tomorrow); err != nil {
	// 	logrus.Errorf("Download 报错:\t%v", err)
	// 	return
	// }
	...
	// userPubHash2, err := script.GetPubKeyHash(address2)
	// if err != nil {

	// 	return
	// }

	// if err := download.Download(fs.DB(), fs.Registry(), fs.Pool(), file.FileID, file.FileKey, userPubHash2); err != nil {
	// 	logrus.Errorf("Download 报错:\t%v", err)
	// 	return
	// }

	// logrus.Printf("开始删除任务\t%s\n\n", file.FileID)

	// if err := delete.Delete(fs.P2P(), fs.Pubsub(), fs.DB(), file.FileID, privateKey); err != nil {
	// 	logrus.Errorf("Delete 报错:\t%v", err)
	// 	return
	// }
	// logrus.Printf("删除任务执行完毕\t%s\n\n", file.FileID)
