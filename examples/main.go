...
func main() {
	...
	if err != nil {
		logrus.Errorf("NewDeP2P 报错:\t%v", err)
		return
	}

	pubsub, err := NewDeP2PPubSub(ctx, p2p)
	if err != nil {
		logrus.Errorf("NewDeP2PPubSub 报错:\t%v", err)
		return
	}

	// 设置一个推荐选项列表以获得良好的性能
	opt := defs.DefaultOptions()
	// 设置是否启动本地存储选项
	opt.BuildLocalStorage(true)
	//opt.BuildLocalStorage(false)
	opt.BuildRoutingTableLow(1)
	// opt.BuildRootPath("/Users/qinglong/go/src/chaincodes/DeP2P/DeFS/")
	...
	logrus.Print("\n\n返回值:\n")
	logrus.Printf("AssetID:\t%v\n", file.AssetID)
	logrus.Printf("FileHash:\t%v\n", file.FileHash)
	...
	if err := fs.Download(file.AssetID, file.FileHash); err != nil {
...
const (
	...
	RendezvousString = "rendezvous:wesign.xyz.021"
)
...
func GenerateECDSAKeyPair(password []byte, salt []byte, iterations, keyLength int, useSHA512 bool) (*ecdsa.PrivateKey, []byte, error) {
	...
	combined := append([]byte("DeP2P"), salt...)
